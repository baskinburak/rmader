...
void Rmader::clearNonAgentTrajs() {
    mtx_trajs_.lock();

    trajs_.erase(std::remove_if(trajs_.begin(), trajs_.end(),
                                [](mt::dynTrajCompiled const& traj) {
                                    return !traj.is_agent;
                                }),
                 trajs_.end());
...
Rmader::Rmader(mt::parameters par) : par_(par) {
    ...
    if (nlopt_utils::checkGradientsNlopt(par_.basis) == false) {
        std::cout << "=============================================="
                  << std::endl;
        std::cout << bold << "Gradient check was " << red << "NOT OK " << reset
                  << std::endl;
        std::cout << "=============================================="
                  << std::endl;
        ...
        std::cout << "=============================================="
                  << std::endl;
        std::cout << bold << "Gradient check was " << green << " OK " << reset
                  << std::endl;
        std::cout << "=============================================="
                  << std::endl;
    }
    ...
    if (par.basis == "MINVO") {
        ...
        // std::cout << red << "Basis " << par.basis << " not implemented yet"
        // << reset << std::endl; std::cout << red <<
        // "============================================" << reset << std::endl;
        abort();
    }
...
void Rmader::dynTraj2dynTrajCompiled(const mt::dynTraj& traj,
                                     mt::dynTrajCompiled& traj_compiled) {
    traj_compiled.bbox = traj.bbox;
    traj_compiled.id = traj.id;
    traj_compiled.time_received = traj.time_received;
...
void Rmader::updateTrajObstacles_with_delaycheck(mt::dynTraj traj,
    ...
    if (traj.is_committed) {  // if that object already exists, substitute its
                              // trajectory
        // clean
        ...
        trajs_.erase(std::remove_if(trajs_.begin(), trajs_.end(),
                                    [&](mt::dynTrajCompiled const& traj) {
                                        return traj.id == traj_compiled.id;
                                    }),
                     trajs_.end());

        // std::cout << "traj_compiled.id is " << traj_compiled.id << std::endl;
        // for (auto traj : trajs_){std::cout << "traj.id is " << traj.id <<
        // std::endl;}

        trajs_.push_back(traj_compiled);
    } else {  // if it doesn't exist or traj.is_committed == false, then add it
              // to the local map
        trajs_.push_back(traj_compiled);
        // std::cout << "traj_compiled.id is " << traj_compiled.id << std::endl;
        // for (auto traj : trajs_){std::cout << "traj.id is " << traj.id <<
        // std::endl;} ROS_WARN_STREAM("Adding " << traj_compiled.id);
    }

    // and now let's delete those trajectories of the obs/agents whose current
    // positions are outside the local map Note that these positions are
    // obtained with the trajectory stored in the past in the local map
    ...
    for (auto traj_compiled : trajs_) {
        ...
        if (!traj_compiled.is_agent) {
            center_obs =
                traj_compiled.traj
                    ->eval(current_timestamp - traj_compiled.time_created, 0)
                    .value();
        } else {
            center_obs = traj_compiled.pwp.eval(current_timestamp);
        }

        mtx_t_.unlock();

        if (((traj_compiled.is_static == true) &&
             (center_obs - state_.pos).norm() > 2 * par_.Ra) ||  ////
            ((traj_compiled.is_static == false) &&
             (center_obs - state_.pos).norm() > 4 * par_.Ra))
        // #### Static Obstacle: 2*Ra because: traj_{k-1} is inside a sphere of
        // Ra. Then, in iteration k the point A (which I don't know yet)  is
        // taken along that trajectory, and another trajectory of radius Ra will
        // be obtained. Therefore, I need to take 2*Ra to make sure the extreme
        // case (A taken at the end of traj_{k-1} is covered).
    ...
    //       std::cout<< "     is_committed: " << traj_compiled.is_committed <<
    //       std::endl;
    //    }
    //   }
    // }

    // std::cout << "bef mtx_trajs_.unlock() in updateTrajObstacles" <<
    // std::endl;
    mtx_trajs_.unlock();
    // std::cout << "aft mtx_trajs_.unlock() in updateTrajObstacles" <<
    // std::endl;
}

void Rmader::updateTrajObstacles(mt::dynTraj traj, double current_timestamp) {
    MyTimer tmp_t(true);
    ...
    std::vector<mt::dynTrajCompiled>::iterator obs_ptr =
        std::find_if(trajs_.begin(), trajs_.end(),
                     [=](const mt::dynTrajCompiled& traj_compiled) {
                         return traj_compiled.id == traj.id;
                     });
    ...
    if (exists_in_local_map) {  // if that object already exists, substitute its
                                // trajectory
        *obs_ptr = traj_compiled;
    } else {  // if it doesn't exist, add it to the local map
        trajs_.push_back(traj_compiled);
    ...
    // std::cout << bold << blue << "updateTrajObstacles took " << tmp_t <<
    // reset << std::endl;
}

std::vector<Eigen::Vector3d> Rmader::vertexesOfInterval(
    mt::PieceWisePol& pwp, double t_start, double t_end,
    const Eigen::Vector3d& delta) {
    std::vector<Eigen::Vector3d> points;

    std::vector<double>::iterator low =
        std::lower_bound(pwp.times.begin(), pwp.times.end(), t_start);
    std::vector<double>::iterator up =
        std::upper_bound(pwp.times.begin(), pwp.times.end(), t_end);
    ...
    int index_first_interval =
        low - pwp.times.begin() - 1;  // index of the interval [1,2]
    int index_last_interval =
        up - pwp.times.begin() - 1;  // index of the interval [5,6]
    ...
    for (int i = index_first_interval; i <= index_last_interval; i++) {
        ...
        for (int j = 0; j < V.cols(); j++) {
            ...
            if (delta.norm() < 1e-6) {  // no inflation
                points.push_back(Eigen::Vector3d(x, y, z));
            } else {
                // points.push_back(Eigen::Vector3d(V(1, j), V(2, j), V(3, j)));
                // // x,y,z
...
// return a vector that contains all the vertexes of the polyhedral approx of an
// interval.
std::vector<Eigen::Vector3d> Rmader::vertexesOfInterval(
    mt::dynTrajCompiled& traj, double t_start, double t_end) {
    Eigen::Vector3d delta = Eigen::Vector3d::Zero();
    Eigen::Vector3d drone_boundarybox = par_.drone_bbox;

    if (traj.is_agent == false) {
        std::vector<Eigen::Vector3d> points;
        // delta = traj.bbox / 2.0 + (par_.drone_radius + par_.beta +
        // par_.alpha) *
        //                            Eigen::Vector3d::Ones();  // every side of
        //                            the box will be increased by 2*delta
        //(+delta on one end, -delta on the other)

        // changeBBox(drone_boundarybox);

        delta = traj.bbox / 2.0 + drone_boundarybox / 2.0 +
                (par_.beta + par_.alpha + traj.alpha) * Eigen::Vector3d::Ones();
        ...
        // Will always have a sample at the beginning of the interval, and
        // another at the end.
        for (double t = t_start;  /////////////
             (t < t_end) ||       /////////////
             ((t > t_end) &&
              ((t - t_end) < par_.gamma));  /////// This is to ensure we have a
                                            /// sample a the end
             t = t + par_.gamma) {
            mtx_t_.lock();
            t_ = std::min(
                t, t_end);  // this min only has effect on the last sample

            const auto pt =
                traj.traj
                    ->eval(std::max(0.0, t_ - traj.time_created - 1e-10), 0)
                    .value();
            ...
            //"Minkowski sum along the trajectory: box centered on the
            // trajectory"
            points.push_back(
                Eigen::Vector3d(x + delta.x(), y + delta.y(), z + delta.z()));
            points.push_back(
                Eigen::Vector3d(x + delta.x(), y - delta.y(), z - delta.z()));
            points.push_back(
                Eigen::Vector3d(x + delta.x(), y + delta.y(), z - delta.z()));
        ...
        // delta = traj.bbox / 2.0 + (par_.drone_radius) *
        // Eigen::Vector3d::Ones(); delta = traj.bbox / 2.0 + par_.drone_bbox
        // / 2.0;  // instad of using drone_radius

        // changeBBox(drone_boundarybox);

        delta = traj.bbox / 2.0 + drone_boundarybox / 2.0 +
                (par_.beta + par_.alpha) * Eigen::Vector3d::Ones();
        // std::cout << "boundary box size" << std::endl;
        // std::cout << drone_boundarybox[0] << std::endl;
        // std::cout << drone_boundarybox[1] << std::endl;
        // std::cout << drone_boundarybox[2] << std::endl;
        ...
        // std::endl; std::cout << "****Inflation by delta= " <<
...
void Rmader::changeBBox(Eigen::Vector3d& drone_boundarybox) {
    Eigen::Vector3d v(1e-5, 1e-5, 1e-5);  // how much you make the box small?
    Eigen::Vector3d bbox_min_possible(par_.drone_bbox[0] - 0.02,
                                      par_.drone_bbox[1] - 0.02,
                                      par_.drone_bbox[2] - 0.02);
    // double bbox_change_time = 1; //seconds, used for timer
    double unstuck_dist = 1.0;  // meters
    Eigen::Vector3d dist(
        1e5, 1e5, 1e5);  // random big numer, need to initialized before used
    ...
    if (par_.is_stuck || if_bbox_change_ || is_A_star_failed_30_) {
        ...
        // std::cout << "stuck_state_for_bbox_ is " << stuck_state_for_bbox_.pos
        // << std::endl;

        // if (timer_bbox_.ElapsedMs() < bbox_change_time * 1000){ //using timer
        // if (!par_.is_stuck){ // TODO: measure the progress like position and
        // if the drone moves certain distance, put the bbox size back

        if (dist.norm() < unstuck_dist) {
            drone_boundarybox =
                par_.drone_bbox - (stuck_count_for_bbox_ + 1) * v;
            // std::cout << "drone_bbox[0] is " << drone_boundarybox[0] <<
            // std::endl; std::cout << "drone_bbox[1] is " <<
            // drone_boundarybox[1] << std::endl; std::cout << "drone_bbox[2] is
            // " << drone_boundarybox[2] << std::endl;

            for (int i = 0; i < 3; i++) {
                drone_boundarybox[i] =
                    std::max(drone_boundarybox[i], bbox_min_possible[i]);
            };
            // std::cout << "using smaller bbox" << std::endl;
            // std::cout << "stuck count is " << stuck_count_for_bbox_ <<
            // std::endl; std::cout << "par_.is_stuck is " << par_.is_stuck <<
            // std::endl;
...
CGAL_Polyhedron_3 Rmader::convexHullOfInterval(mt::dynTrajCompiled& traj,
                                               double t_start, double t_end) {
    // std::cout << "1.1.4" << std::endl;

    std::vector<Eigen::Vector3d> points =
        vertexesOfInterval(traj, t_start, t_end);
...
void Rmader::removeTrajsThatWillNotAffectMe(const mt::state& A, double t_start,
                                            double t_end) {
    ...
    pointsB.push_back(Eigen::Vector3d(
        A.pos.x() + delta.x(), A.pos.y() + delta.y(), A.pos.z() + delta.z()));
    pointsB.push_back(Eigen::Vector3d(
        A.pos.x() + delta.x(), A.pos.y() - delta.y(), A.pos.z() - delta.z()));
    pointsB.push_back(Eigen::Vector3d(
        A.pos.x() + delta.x(), A.pos.y() + delta.y(), A.pos.z() - delta.z()));
    ...
    for (auto traj : trajs_) {
        ...
        if (traj.is_static == true) {
            ...
            Eigen::Vector3d center_obs;
            if (traj.is_agent == false) {
                center_obs = traj.traj->eval(t_ - traj.time_created, 0).value();
            } else {
            ...
            positive_half_diagonal << traj.bbox[0] / 2.0, traj.bbox[1] / 2.0,
                traj.bbox[2] / 2.0;
        ...
        } else {  // DYNAMIC OBSTACLES/AGENTS
            double deltaT =
                (t_end - t_start) /
                (1.0 * par_.num_pol);  // num_pol is the number of intervals
            for (int i = 0; i <= par_.num_pol; i++)  // for each interval
            {
                std::vector<Eigen::Vector3d> pointsA = vertexesOfInterval(
                    traj, t_start + i * deltaT, t_start + (i + 1) * deltaT);
                // Now we check whether the bbox centered in A (and
                // total_sizes=2*Ra) collides with the polyhedron whose vertexes
                // are pointsA

                // To do that, we solve a linear feasibility program to find a
                // plane that separates that box and the polyhedron

                Eigen::Vector3d n;
                double d;

                bool solved =
                    separator_solver_->solveModel(n, d, pointsA, pointsB);
    ...
    exit:
        ...
        if (traj_affects_me == false) {
            // std::cout << red << bold << "Going to  delete traj " <<
            // trajs_[index_traj].id << reset << std::endl;
    ...
    /*  std::cout << "After deleting the trajectory, we have these ids= " <<
      std::endl;
...
bool Rmader::IsTranslating() {
    return (drone_status_ == DroneStatus::GOAL_SEEN ||
            drone_status_ == DroneStatus::TRAVELING);
}

ConvexHullsOfCurve Rmader::convexHullsOfCurve(mt::dynTrajCompiled& traj,
                                              double t_start, double t_end) {
    ConvexHullsOfCurve convexHulls;
    double deltaT = (t_end - t_start) /
                    (1.0 * par_.num_pol);  // num_pol is the number of intervals

    // std::cout << "1.1.3" << std::endl;

    for (int i = 0; i <= par_.num_pol; i++) {
        convexHulls.push_back(convexHullOfInterval(traj, t_start + i * deltaT,
                                                   t_start + (i + 1) * deltaT));
    }
...
void Rmader::setTerminalGoal(mt::state& term_goal) {
    ...
    // G_term_.pos.transpose() << reset << std::endl; std::cout << bold << red
    // << "[FA] Received Proj Goal=" << G_.pos.transpose() << reset <<
    // std::endl;
...
mt::state Rmader::getGterm() { return G_term_; }

void Rmader::getG(mt::state& G) { G = G_; }

// in case drones are stuck we need a new G to detour
void Rmader::getDetourG(mt::state& G) {
    ...
    // G.pos.head(2) = stuck_state_.pos.head(2) + v2_mag * v2; // for 180 deg
    // rotation
    detoured_G_.pos.head(2) = stuck_state_.pos.head(2) + v2_mag * v2;
    detoured_G_.pos[2] = G_when_stuck_.pos[2];

    // if new G is outside of highbay boundary, the project new G to highbay
    // boundary
    ...
    //        |
    //        |            v
    //        G <----------------------- A (drone's current initial planning
    //        position)
    //
    //
}

void Rmader::moveAtowardG(mt::state& A, mt::state& G) {
...
Eigen::Vector2d Rmader::RotationMatrix(Eigen::Vector2d& vec,
                                       const double& angle) {
    // angle is radian
    Eigen::Vector2d vec2;
...
void Rmader::getState(mt::state& data) {
    mtx_state.lock();
    data = state_;
    mtx_state.unlock();
}

void Rmader::updateState(mt::state data) {
    state_ = data;
...
bool Rmader::initializedAllExceptPlanner() {
    if (!state_initialized_ || !terminal_goal_initialized_) {
        /*    std::cout << "state_initialized_= " << state_initialized_ <<
           std::endl; std::cout << "terminal_goal_initialized_= " <<
           terminal_goal_initialized_ << std::endl;*/
        return false;
    }
    return true;
}

bool Rmader::initializedStateAndTermGoal() {
    if (!state_initialized_ || !terminal_goal_initialized_) {
        if (!state_initialized_) {
...
bool Rmader::initialized() {
    if (!state_initialized_ || !terminal_goal_initialized_ ||
        !planner_initialized_) {
        /*    std::cout << "state_initialized_= " << state_initialized_ <<
           std::endl; std::cout << "terminal_goal_initialized_= " <<
           terminal_goal_initialized_ << std::endl; std::cout <<
           "planner_initialized_= " << planner_initialized_ << std::endl;*/
        return false;
    }
    return true;
}

std::vector<mt::dynTrajCompiled> Rmader::getTrajs() {
...
// check wheter a mt::dynTrajCompiled and a pwp_optimized are in collision in
// the interval [t_start, t_end]
bool Rmader::trajsAndPwpAreInCollision_with_inflation(
    mt::dynTrajCompiled traj, mt::PieceWisePol pwp_optimized, double t_start,
    ...
    for (int i = 0; i < par_.num_pol; i++)  // for each interval
    {
        ...
        //     vertexesOfInterval(pwp_optimized, t_start + i * deltaT, t_start +
        //     (i + 1) * deltaT, Eigen::Vector3d::Zero());
        ...
        std::vector<Eigen::Vector3d> pointsA =
            vertexesOfInterval(pwp_optimized, t_start + i * deltaT,
                               t_start + (i + 1) * deltaT, inflation);

        // This is the trajectory of the other agent/obstacle
        std::vector<Eigen::Vector3d> pointsB = vertexesOfInterval(
            traj, t_start + i * deltaT, t_start + (i + 1) * deltaT);

        // std::cout << "Going to solve model with pointsA.size()= " <<
        // pointsA.size() << std::endl; for (auto point_i : pointsA)
        // {
        //   std::cout << point_i.transpose() << std::endl;
        // }

        // std::cout << "Going to solve model with pointsB.size()= " <<
        // pointsB.size() << std::endl; for (auto point_i : pointsB)
        // {
        //   std::cout << point_i.transpose() << std::endl;
        // }

        if (separator_solver_->solveModel(n_i, d_i, pointsA, pointsB) ==
            false) {
            if (i == 0) {
                is_q0_fail = true;
            }
...
bool Rmader::trajsAndPwpAreInCollision(mt::dynTrajCompiled traj,
    ...
    for (int i = 0; i < par_.num_pol; i++)  // for each interval
    {
        // This is my trajectory (no inflation)
        std::vector<Eigen::Vector3d> pointsA = vertexesOfInterval(
            pwp_optimized, t_start + i * deltaT, t_start + (i + 1) * deltaT,
            Eigen::Vector3d::Zero());
...
// Checks that I have not received new trajectories that affect me while doing
// the optimization Check period and Recheck period is defined here
bool Rmader::safetyCheckAfterOptForRmader(mt::PieceWisePol pwp_optimized,
                                          bool& is_q0_fail) {
    bool result = true;
    for (auto& traj : trajs_) {
        if (traj.is_agent == true)  // need to include the trajs that came in
                                    // the last delay check
        {
            if (trajsAndPwpAreInCollision_with_inflation(
                    traj, pwp_optimized, pwp_optimized.times.front(),
...
bool Rmader::safetyCheckAfterOpt(mt::PieceWisePol pwp_optimized) {
    ...
    for (auto traj : trajs_) {
        if (traj.time_received > time_init_opt_ && traj.is_agent == true) {
            if (trajsAndPwpAreInCollision(traj, pwp_optimized,
                                          pwp_optimized.times.front(),
    ...
    // and now do another check in case I've received anything while I was
    // checking. Note that mtx_trajs_ is locked!
...
bool Rmader::delayCheck(mt::PieceWisePol pwp_now, const double& headsup_time,
    ...
    mtx_trajs_.lock();  // this function is called in mader_ros.cpp so need to
    ...
    for (auto& traj_compiled : trajs_) {
        if (traj_compiled.is_agent == true) {
            if (par_.is_optimistic_dc) {
                if (!traj_compiled.is_committed) {
                    // std::cout << "headsup_time " << headsup_time <<
                    // std::endl; std::cout << "time created " <<
                    // traj_compiled.time_created << std::endl;
                    if (traj_compiled.time_created - headsup_time > 1e-2) {
                        // Do nothing. They will change their traj.
                               ...
                               1e-2) {
                        if (trajsAndPwpAreInCollision_with_inflation(
                                traj_compiled, pwp_now, pwp_now.times.front(),
                                pwp_now.times.back(), is_q0_fail)) {
                            ROS_ERROR_STREAM(
                                "In delay check traj_compiled collides with "
                                << traj_compiled.id);
                            result =
                                false;  // will have to redo the optimization
                        }
                    } else if (abs(traj_compiled.time_created - headsup_time) <
                               1e-2) {
                        if (trajsAndPwpAreInCollision_with_inflation(
                                traj_compiled, pwp_now, pwp_now.times.front(),
                                pwp_now.times.back(), is_q0_fail)) {
                                ...
                                center_obs =
                                    traj_compiled.traj
                                        ->eval(current_timestamp -
                                                   traj_compiled.time_created,
                                               0)
                                        .value();
                            } else {
                                center_obs =
                                    traj_compiled.pwp.eval(current_timestamp);
                            }
                            ...
                            // center_obs[0] == state_.pos[0] &&  center_obs[1]
                            // == state_.pos[1] &&  center_obs[2] ==
                            // state_.pos[2] won't happen bc it's the same
            ...
            } else {  // if traj_compiled.is_agent == false
                if (trajsAndPwpAreInCollision_with_inflation(
                        traj_compiled, pwp_now, pwp_now.times.front(),
                        pwp_now.times.back(), is_q0_fail)) {
                    ROS_ERROR_STREAM(
        ...
        //             traj_compiled, pwp_now, pwp_now.times.front(),
        //             pwp_now.times.back(), is_q0_fail)) {
        //         ROS_ERROR_STREAM("In delay check traj_compiled collides with "
        //                          << traj_compiled.id);
        //         result = false;
        //     }
        // }
...
bool Rmader::safetyCheck_for_A_star_failure(mt::PieceWisePol pwp_prev) {
    ...
    for (auto traj : trajs_) {
        if (traj.time_received > time_init_opt_ && traj.is_agent == true) {
            if (trajsAndPwpAreInCollision(traj, pwp_prev,
                                          pwp_prev.times.front(),
                                          pwp_prev.times.back())) {
                if (id_ < traj.id) {  // tie breaking mechanism
                    ROS_ERROR_STREAM("my previous traj collides with "
                                     << traj.id
                                     << " and they have higher number, so I "
                                        "need to change my traj.");
                    result = false;  // will have to redo the optimization
                }
                break;
            }
...
std::vector<Rmader::VectorDIM> Rmader::getPlan() {
    std::vector<Rmader::VectorDIM> plan;
    for (auto& p : plan_) {
        plan.push_back(p.pos);
    }
    if (plan.size() == 1) {
...
bool Rmader::safetyCheck_for_A_star_failure_pwp_now(mt::PieceWisePol pwp_now) {
    ...
    for (auto traj : trajs_) {
        if (traj.time_received > time_init_opt_ && traj.is_agent == true) {
            if (trajsAndPwpAreInCollision(traj, pwp_now, pwp_now.times.front(),
                                          pwp_now.times.back())) {
                if (id_ < traj.id) {  // tie breaking mechanism
                    ROS_ERROR_STREAM("my pwp_now collides with "
                                     << traj.id
                                     << " and they have higher number, so I "
                                        "need to change my traj.");
                    result = false;  // will have to redo the optimization
...
bool Rmader::isReplanningNeeded() {
    ...
    // std::cout << "dist_last_plan_to_goal= " << dist_last_plan_to_goal <<
    // std::endl;
    mtx_plan_.unlock();
...
bool Rmader::replan_with_delaycheck(mt::Edges& edges_obstacles_out,
                                    std::vector<mt::state>& headsup_plan,
    ...
    //   // following popped up trajectory and haven't yet reached to
    //   pop_up_last_state_in_plan_ Eigen::Vector3d diff = state_.pos -
    //   pop_up_last_state_in_plan_.pos; pwp_out =
    //   mu::constPosition2pwp(pop_up_last_state_in_plan_.pos); if (diff.norm()
    //   > 0.1){
    //     return true;
    //   }
    ...
    // if (is_z_max_increased_ && is_going_back_to_normal_z_max_){
    //   // once you reached pop_up_last_state_in_plan_, then you start planning
    //   new traj in extended z_max space
    ...
    //   (state_.pos[2] < par_.z_max - 0.3){ // if you go back to the nominal
    //   z_max, then put z_max back. 0.3 is a buffer
    //     std::cout << "got back to the nomial space!!" << "\n";
    //     solver_->changeZmax(par_.z_max); // put the z_max back to the nominal
    //     value is_z_max_increased_ = false; is_going_back_to_normal_z_max_ =
    //     false;
    //   }
    // }
    // std::cout << "par_for_solver.z_max " << solver_->printZmax() << "\n";
    ...
    mu::saturate(deltaT_, par_.delay_check / par_.dc,
                 par_.delay_check / par_.dc);

    k_index_end_ = std::max((int)(plan_.size() - deltaT_ - 2), 0);
    ...
    // std::cout << blue << "k_index_end_:" << k_index_end_ << reset <<
    // std::endl; std::cout << blue << "plan_.size():" << plan_.size() << reset
    // << std::endl;
    ...
    // if (k_index_end_ != 0) {
    //     runtime_snlopt =
    //         k_index * par_.dc;  // std::min(,
    //         par_.upper_bound_runtime_snlopt);
    // } else {
    runtime_snlopt = par_.upper_bound_runtime_snlopt;  // I'm stopped at the end
                                                       // of the trajectory -->
                                                       // take my time to replan
    // }
    // mu::saturate(runtime_snlopt, par_.lower_bound_runtime_snlopt,
    //              par_.upper_bound_runtime_snlopt);

    // std::cout << green << "Runtime snlopt= " << runtime_snlopt << reset <<
    // std::endl;
    ...
    double ra =
        std::min((distA2TermGoal - 0.001), par_.Ra);  // radius of the sphere S
    mt::state G;
    ...
    // double reached_goal_dist = par_.drone_bbox[0]+0.1; // avoid the detoured
    // goal is within others bbox and cannot move at all sitution. double
    // how_much_to_detoured_G = 0.7; //0.1 means 10% double vel_stuck_detect =
    // 0.1; // m/s
    ...
    //   // std::cout << "stuck_state_.pos is " << stuck_state_.pos <<
    //   std::endl;
    ...
    //       if_detour_ = false; // maybe the detour G is also causing stuck, so
    //       go back to the original one if_A_moveback_ = false;
    //     }
    //   } else {
    ...
    //   //   std::cout << "dist_prog.norm() is " << dist_prog.norm() <<
    //   std::endl;
    //   }
    // }
    ...
    //////////////////////
    time_now_ =
        current_timestamp;  // TODO this ros dependency shouldn't be here

    double t_start = k_index * par_.dc + time_now_;

    double factor_alloc = (distA2TermGoal > par_.dist_factor_alloc_close)
                              ? par_.factor_alloc
                              : par_.factor_alloc_close;

    double time_allocated =
        factor_alloc *
        mu::getMinTimeDoubleIntegrator3D(initial.pos, initial.vel, final.pos,
    ...
    bool correctInitialCond = solver_->setInitStateFinalStateInitTFinalT(
        initial, final, t_start,
        t_final);  // note that here t_final may have been updated

    if (correctInitialCond == false) {
        std::cout << bold << red
                  << "The solver cannot guarantee feasibility for v1" << reset
                  << std::endl;
        return false;
    }
    ...
    time_init_opt_ = current_timestamp;
    ...
    mt::ConvexHullsOfCurves_Std hulls_std =
        cu::vectorGCALPol2vectorStdEigen(hulls);
    ...
    //////////////////////
    // std::cout << on_cyan << bold << "Solved so far" << solutions_found_ <<
    // "/" << total_replannings_ << reset
    //           << std::endl;
    ...
    bool is_q0_fail = false;
    bool result = solver_->optimize(is_stuck, is_A_star_failed,
                                    is_q0_fail);  // calling the solver

    // if (!result){
    //   // std::cout << "q0_fail_count_ is " << q0_fail_count_ << std::endl;
    //   opt_fail_count_ += 1;
    //   if (opt_fail_count_ > 3 && state_.vel.norm() < 0.001){
    //     // in this case one agent is on the bbox constraint so need to move A
    //     away std::cout << "[opt fail] move A out of the bbox" << std::endl;
    ...
    // sometimes the very initial path search takes more than
    // how_many_A_star_failure counts and fails if (planner_initialized_){
    ...
    //     // need to check if my previous traj collides with others. and if
    //     that's the case pop it up std::cout << "A_star is failing\n";
    ...
    //         // this only happens when two agents commit traj at the very same
    //         time (or in Recheck period) is_pop_up_ = true; A_star_fail_count_
    //         = 0; return false; //abort mader
    //       } else {
    ...
    if (result == false) {
        int states_last_replan =
            ceil(replanCB_t.ElapsedMs() / (par_.dc * 1000) +
                 par_.delay_check * par_.comm_delay_param /
                     par_.dc);  // Number of states that
    ...
    // av_improvement_nlopt_ = ((solutions_found_ - 1) * av_improvement_nlopt_ +
    // solver_->improvement_) / solutions_found_;

    // std::cout << blue << "Average improvement so far" << std::setprecision(5)
    // << av_improvement_nlopt_ << reset
    //          << std::endl;
    ...
    //     // need to check if my previous traj collides with others. and if
    //     that's the case pop it up A_star_fail_count_pwp_now_ =
    //     A_star_fail_count_pwp_now_ + 1; std::cout << "A_star is failing\n";
    //     std::cout << "A_star_fail_count_pwp_now_ is " <<
    //     A_star_fail_count_pwp_now_ << "\n"; double
    //     how_many_A_star_failure_now = 30; if (A_star_fail_count_pwp_now_ >
    //     how_many_A_star_failure_now){
    //       if(!safetyCheck_for_A_star_failure_pwp_now(pwp_now)){
    //         std::cout << "pwp now collide!" << "\n";
    //         // if previous pwp is not feasible pop up the drone
    //         // this only happens when two agents commit traj at the very same
    //         time (or in Recheck period) is_pop_up_ = true; return false;
    //         //abort mader
    //       } else {
    //         is_pop_up_ = false;
    ...
    if (par_.is_check) {
        // std::cout << "bef mtx_trajs_.lock() in replan (safetyCheckAfterOpt)"
        // << std::endl;
        mtx_trajs_.lock();
        // std::cout << "aft mtx_trajs_.lock() in replan (safetyCheckAfterOpt)"
        // << std::endl;

        // first make sure none of the trajectory is checked in this
        // optimization for (auto &traj : trajs_){
        //   traj.is_checked = false;
        // }

        // check and recheck are done in safetyChechAfterOpt()

        bool is_safe_after_opt =
            safetyCheckAfterOptForRmader(pwp_now, is_q0_fail);

        // std::cout << "bef mtx_trajs_.unlock() in replan
        // (safetyCheckAfterOpt)" << std::endl;
        mtx_trajs_.unlock();
        headsup_time = current_timestamp;
        // std::cout << "aft mtx_trajs_.unlock() in replan
        // (safetyCheckAfterOpt)" << std::endl;

        // if (!is_safe_after_opt){
        //   // std::cout << "q0_fail_count_ is " << q0_fail_count_ <<
        //   std::endl; safetycheck_fail_count_ += 1; if
        //   (safetycheck_fail_count_ > 3 && state_.vel.norm() < 0.001){
        //     // in this case one agent is on the bbox constraint so need to
        //     move A away std::cout << "[safety check fail] move A out of the
        //     bbox" << std::endl; movedA_ = moveAoutOfBbox(A);
        //     is_movingAoutOfBbox_ = true;
        //   }
        // } else {
        //   q0_fail_count_ = 0;
        ...
        if (is_safe_after_opt == false) {
            // int states_last_replan = ceil(replanCB_t.ElapsedMs() / (par_.dc *
            // 1000) +
            //                               par_.delay_check *
            //                               par_.comm_delay_param / par_.dc);
    ...
    if ((headsup_plan_size - 1 - k_index_end_) < 0) {
        std::cout << bold << red << "Already published the point A" << reset
                  << std::endl;
        ...
        headsup_plan.erase(
            headsup_plan.end() - k_index_end_ - 1,
            headsup_plan.end());  // this deletes also the initial condition...
        for (int i = 0; i < (solver_->traj_solution_).size();
             i++)  //... which is included in traj_solution_[0]
        {
...
bool Rmader::addTrajToPlan_with_delaycheck(mt::PieceWisePol& pwp) {
    ...
    if ((plan_size - 1 - k_index_end_) < 0) {
        ...
        plan_.erase(plan_.end() - k_index_end_ - 1,
                    plan_.end());  // this deletes also the initial condition...
        // std::cout << "middle, plan_size=" << plan_.size() << " sol.size()="
        // << (solver_->traj_solution_).size()
        // << std::endl;
        for (int i = 0; i < (solver_->traj_solution_).size();
             i++)  //... which is included in traj_solution_[0]
        {
    ...
    is_pwp_prev_feasible_ =
        false;  // we don't know for sure if this traj is feasible until you run
                // opt in the next step and see A* fails

    return true;
}

void Rmader::resetInitialization() {
...
void Rmader::getDesiredYaw(mt::state& next_goal) {
    if (par_.is_camera_yawing) {
        ...
        double desired_yaw = atan2(state_.pos[1], state_.pos[0] - 5.5) -
                             M_PI / 2;  // - M_PI / 2 is because the camera is
                                        // mounted pointing at y-axis
    ...
    switch (drone_status_)
    {
      case DroneStatus::YAWING:
        desired_yaw = atan2(G_term_.pos[1] - next_goal.pos[1], G_term_.pos[0] -
    next_goal.pos[0]); diff = desired_yaw - state_.yaw; break; case
    DroneStatus::TRAVELING: case DroneStatus::GOAL_SEEN: desired_yaw =
    atan2(M_.pos[1] - next_goal.pos[1], M_.pos[0] - next_goal.pos[0]); diff =
    desired_yaw - state_.yaw; break; case DroneStatus::GOAL_REACHED:
        next_goal.dyaw = 0.0;
        next_goal.yaw = previous_yaw_;
        return;
    }
...
bool Rmader::peekNextGoal(mt::state& next_goal) {
    if (initializedStateAndTermGoal() ==
        false)  // || (drone_status_ == DroneStatus::GOAL_REACHED &&
                // plan_.size() == 1))
    {
...
bool Rmader::getNextGoal(mt::state& next_goal) {
    ...
    //     plan_.content[i].pos[2] = plan_.content[i].pos[2] + i *
    //     pop_up_increment;
    //     }
    //   } else {
...
void Rmader::getID(int& id) { id_ = id; }